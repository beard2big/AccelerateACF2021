...
function New-AdoPipeline {
  ...
  try {
    ...
    $PipelineSourcePath = (Get-Location).Path + $PipelineSourcePath
    ...
    foreach ($pipeline in $pipelinesToBeUpdated) {
      ...
      $pipeline = az pipelines create --project "$($pipeline.ProjectName)" `
        --organization "$orgUrl" `
        ...
        --skip-run
      $pipelineobject = $pipeline | ConvertFrom-Json
      if ($createBuildValidation) {
        $pathFilter = $ymlpath -replace 'pipeline.yml', '*'
        Write-Verbose "----------------------------------"
        ...
        $buildvalidation = az repos policy build create `
          --blocking true `
          --branch master `
          --build-definition-id $pipelineobject.definition.id `
